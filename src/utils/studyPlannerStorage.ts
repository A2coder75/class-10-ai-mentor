...
export function saveStudyPlanToStorage(data: any) {
  try {
    localStorage.setItem('studyPlan', JSON.stringify(data));
    console.log('Study plan saved to storage:', data);
  } catch (error) {
...
export function loadStudyPlanFromStorage() {
  try {
    ...
    console.error('Error loading study plan from storage:', error);
    return null;
  }
}
...
export function getTodaysStudyPlan() {
  try {
    const studyPlan = loadStudyPlanFromStorage();
    if (!studyPlan || !studyPlan.study_plan || studyPlan.study_plan.length === 0) {
      return null;
    }
    ...
    // Find today's plan or the closest upcoming day
    ...
    // If no exact match for today, return the first day
    if (studyPlan.study_plan[0] && studyPlan.study_plan[0].days && studyPlan.study_plan[0].days.length > 0) {
      return {
        ...studyPlan.study_plan[0].days[0],
        weekNumber: 0,
        dayIndex: 0
      };
    }
